...
namespace CatCraft
{
    public class Game
    {
        ...
        private void DrawBag(Graphics g)
        {
            ...
            int bagHeight = (Player.BAG_ROWS + 1) * (slotSize + slotPadding) + slotPadding + 30;
            ...
            // 操作提示
            g.DrawString("点击背包物品放入快捷栏 | 右键出售单个物品", font, new SolidBrush(Color.Gray), bagX + 10, bagY + bagHeight - 20);
        }
        ...
        private void DrawHotbar(Graphics g, int screenWidth, int screenHeight)
        {
            ...
            for (int i = 0; i < Player.HOTBAR_SIZE; i++)
            {
                ...
                if (stack != null)
                {
                    DrawItem(g, stack.TileId, stack.Count, x + 2, y + 16, slotSize - 4);
                }
            }
        }
            ...
            for (int row = 0; row < Player.BAG_ROWS; row++)
            {
                for (int col = 0; col < Player.BAG_COLS; col++)
                {
                        ...
                        mouseY >= y && mouseY <= y + slotSize)
                    {
                        ...
                        {
                            // 左键放入快捷栏
                            if (_player.Bag[slotIndex] != null)
                            {
                                // 找快捷栏中的空格子
                                for (int i = 0; i < Player.HOTBAR_SIZE; i++)
                                {
                                    if (_player.Hotbar[i] == null)
                                    {
                                        _player.MoveToHotbar(slotIndex, i);
                                        DebugConsole.Log($"物品移动到快捷栏 {i + 1}");
                                        break;
                                    }
                                }
                            }
