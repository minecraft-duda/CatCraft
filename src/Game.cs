...
namespace CatCraft
{
    public class Game
    {
        ...
        public string GetTimeDisplay()
        {
            float t = _dayNightCycle;
            if (t < 0.2f) return "🌅 日出";
            if (t < 0.45f) return "☀️ 上午";
            if (t < 0.55f) return "🌤️ 正午";
            if (t < 0.7f) return "🌇 下午";
            if (t < 0.8f) return "🌅 日落";
            return "🌙 夜晚";
        }
        ...
        public (Image Image, string Name, int NextLevel, int Price, bool IsMaxed) GetPickaxeInfo()
        {
            ...
            try
            {
                if (File.Exists(PICKAXE_IMAGES[Math.Min(_player.PickaxeLevel - 1, 5)]))
                    img = Image.FromFile(PICKAXE_IMAGES[Math.Min(_player.PickaxeLevel - 1, 5)]);
            }
            catch { }
        ...
        public Image GetPickaxeCursorImage()
        {
            try
            {
                if (File.Exists(PICKAXE_IMAGES[_player.PickaxeLevel - 1]))
                    return Image.FromFile(PICKAXE_IMAGES[_player.PickaxeLevel - 1]);
            }
            catch { }
            return null;
        }
        ...
        private void NewGame()
		{
			...
			DebugConsole.Log($"新世界创建完成，玩家位置: ({_player.X}, {_player.Y})");
		}
		
		private void LoadGameState(GameSaveData data)
		{
			DebugConsole.Log("加载游戏状态");
			...
			// 从保存数据恢复玩家
			if (data.PlayerX >= 0 && data.PlayerX <= _world.WorldWidth * TILE_SIZE)
			{
				...
				DebugConsole.Log($"加载玩家位置: ({_player.X}, {_player.Y})");
			}
			else
			{
				// 如果保存的位置无效，使用重生点
				...
				DebugConsole.Log($"使用重生点位置: ({_player.X}, {_player.Y})");
			}
			...
			DebugConsole.Log($"游戏状态加载完成 - 位置: ({_player.X}, {_player.Y}), 金钱: {_player.Money}, 血量: {_player.Health}");
		}
